...
class Exp_Informer(Exp_Basic):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            vali_loss = self.vali(vali_data, vali_loader, criterion)  # 计算每个epoch的验证的loss值
            vali_losses.append(vali_loss)  # 将每个epoch的验证的loss值添加到vali_losses列表中
            test_loss = self.vali(test_data, test_loader, criterion)  # 计算每个epoch的测试的loss值
            test_losses.append(test_loss)  # 将每个epoch的测试的loss值添加到test_losses列表中
            ...
            # 绘制当前的loss曲线，并保存到文件中
            plt.figure()  # 创建一个新的图形
            plt.plot(train_losses, label='Train Loss')  # 绘制训练的loss值的折线图，设置标签为Train Loss
            plt.plot(vali_losses, label='Vali Loss')  # 绘制验证的loss值的折线图，设置标签为Vali Loss
            plt.plot(test_losses, label='Test Loss')  # 绘制测试的loss值的折线图，设置标签为Test Loss
            plt.xlabel('Epoch')  # 设置x轴的标签为Epoch
            plt.ylabel('Loss')  # 设置y轴的标签为Loss
            plt.legend()  # 显示图例
            plt.savefig('res/loss_curve.png') # 保存图形到文件中
            plt.close()  # 关闭图形
